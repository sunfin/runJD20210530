...
!(async () => {
  ...
  await getAuthorShareCode('https://raw.githubusercontent.com/sunfin/runJD20210530/master/jd_zz.json');
...
async function getRandomCode() {
  await $.http.get({url: `https://raw.githubusercontent.com/sunfin/runJD20210530/master/jd_zz.json`, timeout: 10000}).then(async (resp) => {
    if (resp.statusCode === 200) {
      try {
        let { body } = resp;
        body = JSON.parse(body);
